...
{
    if (self.text.length < clickTextRange.location+clickTextRange.length) {
        return;
    }
    
    //设置文字颜色
    if (color) {
        [self.content addAttribute:NSForegroundColorAttributeName value:color range:clickTextRange];
    }
    self.attributedText = self.content;
    
    // 设置下划线文字的点击事件
    // self.selectedRange  影响  self.selectedTextRange
    self.selectedRange = clickTextRange;
    ...
    for (UITextSelectionRect *selectionRect in selectionRects) {
        ...
        // 存储下划线对应的文字
        [dic setObject:[self.text substringWithRange:clickTextRange] forKey:@"content"];
        // 存储相应的回调的block
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    for (NSDictionary *dic in selectedArray) {
        if(dic && dic[@"coverColor"]){ //  点击字段的背景UIView
